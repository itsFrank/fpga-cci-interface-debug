...
int AFU_initControl(opaeutils::AFU_Handle* afu_handle, uint64_t* afu_ctrl_ptr, uint64_t* afu_stts_ptr, bool quiet = true) {
    ...
    afu_handle->writeCSR(1, (uint64_t) afu_ctrl_ptr);
    afu_handle->writeCSR(2, (uint64_t) afu_stts_ptr);

    // Wait for AFU to confirm addresses have been recieved
    while (afu_handle->readCSR(1) != (uint64_t)afu_ctrl_ptr){}
    while (afu_handle->readCSR(2) != (uint64_t)afu_stts_ptr){}
    ...
    return afu_handle->readCSR(7); // Return current value of nonce
}
...
void AFU_waitStatus(uint64_t* afu_stts_ptr, AFU_STATUS status_code) {
    ...
    while(current_status != (uint64_t)status_code) {
        ...
        tim.tv_sec  = 1;
        tim.tv_nsec = 500;
        std::cout << "Waiting for FPGA status code: " << (uint64_t)status_code << " - Current value: " << current_status << std::endl;
        nanosleep(&tim, NULL);
    }
}

void AFU_waitState(opaeutils::AFU_Handle* afu_handle, AFU_STATE state) {
    while (afu_handle->readCSR(0) != (uint64_t)state){}
}
